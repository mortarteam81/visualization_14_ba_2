...
BUDAM_CANDIDATE_PATH = (
    PROJECT_ROOT
    / "data"
    / "conversion_outputs"
    / "academyinfo"
    / "budam"
    / "budam_2011_2024_candidate.csv"
)
BUDAM_REPORT_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "processing_reports"
    / "academyinfo_budam.processing_report.json"
)
BUDAM_MISMATCH_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "mismatch_reports"
    / "academyinfo_budam.mismatch.csv"
)
BUDAM_SOURCE_ACQUISITION_PATH = (
    PROJECT_ROOT
    / "data"
    / "raw"
    / "academyinfo"
...
BUDAM_REVIEW_DECISIONS_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "review_decisions"
    / "academyinfo_budam.review.json"
)
...
GYEOLSAN_CANDIDATE_PATH = (
    PROJECT_ROOT
    / "data"
    / "conversion_outputs"
    / "kasfo"
    / "gyeolsan"
    / "kasfo_gyeolsan_2015_2024_candidate.csv"
)
GYEOLSAN_REPORT_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "processing_reports"
    / "kasfo_gyeolsan.processing_report.json"
)
GYEOLSAN_MISMATCH_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "mismatch_reports"
    / "kasfo_gyeolsan.mismatch.csv"
)
GYEOLSAN_SOURCE_ACQUISITION_PATH = (
    PROJECT_ROOT
    / "data"
    / "raw"
    / "kasfo"
...
GYEOLSAN_REVIEW_DECISIONS_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "review_decisions"
    / "kasfo_gyeolsan.review.json"
)
EDUCATION_RETURN_CANDIDATE_PATH = (
    PROJECT_ROOT
    / "data"
    / "conversion_outputs"
    / "kasfo"
    / "education_return"
    / "kasfo_education_return_2016_2025_candidate.csv"
)
EDUCATION_RETURN_REPORT_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "processing_reports"
    / "kasfo_education_return.processing_report.json"
)
EDUCATION_RETURN_MISMATCH_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "mismatch_reports"
    / "kasfo_education_return.mismatch.csv"
)
EDUCATION_RETURN_SOURCE_ACQUISITION_PATH = (
    PROJECT_ROOT
    / "data"
    / "raw"
    / "kasfo"
...
EDUCATION_RETURN_REVIEW_DECISIONS_PATH = (
    PROJECT_ROOT
    / "data"
    / "validation"
    / "review_decisions"
...
def load_budam_candidate_frame() -> pd.DataFrame:
    return pd.read_csv(BUDAM_CANDIDATE_PATH, encoding="utf-8-sig")
...
def load_gyeolsan_candidate_frame() -> pd.DataFrame:
    return pd.read_csv(GYEOLSAN_CANDIDATE_PATH, encoding="utf-8-sig")


def load_education_return_candidate_frame() -> pd.DataFrame:
...
def load_budam_mismatch_frame() -> pd.DataFrame:
    if not BUDAM_MISMATCH_PATH.exists():
        return pd.DataFrame()
    return pd.read_csv(BUDAM_MISMATCH_PATH, encoding="utf-8-sig")
...
def load_gyeolsan_mismatch_frame() -> pd.DataFrame:
    if not GYEOLSAN_MISMATCH_PATH.exists():
        return pd.DataFrame()
    return pd.read_csv(GYEOLSAN_MISMATCH_PATH, encoding="utf-8-sig")


def load_education_return_mismatch_frame() -> pd.DataFrame:
    if not EDUCATION_RETURN_MISMATCH_PATH.exists():
        return pd.DataFrame()
...
def load_budam_processing_report() -> dict[str, Any]:
    return _read_json(BUDAM_REPORT_PATH)
...
def load_gyeolsan_processing_report() -> dict[str, Any]:
    return _read_json(GYEOLSAN_REPORT_PATH)


def load_education_return_processing_report() -> dict[str, Any]:
...
def load_budam_source_acquisition() -> dict[str, Any]:
    return _read_json(BUDAM_SOURCE_ACQUISITION_PATH)
...
def load_gyeolsan_source_acquisition() -> dict[str, Any]:
    return _read_json(GYEOLSAN_SOURCE_ACQUISITION_PATH)
...
def load_budam_review_decisions(
    path: Path | str = BUDAM_REVIEW_DECISIONS_PATH,
) -> dict[str, ReviewDecision]:
    return load_dormitory_review_decisions(path)
...
def load_gyeolsan_review_decisions(
    path: Path | str = GYEOLSAN_REVIEW_DECISIONS_PATH,
) -> dict[str, ReviewDecision]:
    return load_dormitory_review_decisions(path)


def load_education_return_review_decisions(
    path: Path | str = EDUCATION_RETURN_REVIEW_DECISIONS_PATH,
) -> dict[str, ReviewDecision]:
    return load_dormitory_review_decisions(path)
...
def save_budam_review_decisions(
    decisions: Mapping[str, ReviewDecision],
    path: Path | str = BUDAM_REVIEW_DECISIONS_PATH,
) -> None:
    save_dormitory_review_decisions(
...
def save_gyeolsan_review_decisions(
    decisions: Mapping[str, ReviewDecision],
    path: Path | str = GYEOLSAN_REVIEW_DECISIONS_PATH,
) -> None:
    save_dormitory_review_decisions(
        decisions,
        path=path,
        dataset_id=GYEOLSAN_DATASET_ID,
    )


def save_education_return_review_decisions(
    decisions: Mapping[str, ReviewDecision],
    path: Path | str = EDUCATION_RETURN_REVIEW_DECISIONS_PATH,
) -> None:
    save_dormitory_review_decisions(
...
def build_budam_validation_status() -> ValidationModeStatus:
    report = load_budam_processing_report()
    candidate_exists = BUDAM_CANDIDATE_PATH.exists()
    report_exists = BUDAM_REPORT_PATH.exists()
    mismatch_exists = BUDAM_MISMATCH_PATH.exists()
    mismatch = load_budam_mismatch_frame()
    ...
    review_status = build_review_completion_status(
        ...
        base_ready=ready_for_preview,
        high_mismatches=high_mismatches,
        dataset_id=str(report.get("dataset_id") or BUDAM_DATASET_ID),
    )

    if not ready_for_preview:
        reason = "원자료 기반 법정부담금 부담율 candidate/report가 아직 완성되지 않았습니다."
...
def build_gyeolsan_validation_status() -> ValidationModeStatus:
    report = load_gyeolsan_processing_report()
    source = load_gyeolsan_source_acquisition()
    candidate_exists = GYEOLSAN_CANDIDATE_PATH.exists()
    report_exists = GYEOLSAN_REPORT_PATH.exists()
    mismatch_exists = GYEOLSAN_MISMATCH_PATH.exists()
    mismatch = load_gyeolsan_mismatch_frame()
    ...
    ready_for_preview = candidate_exists and report_exists and raw_preserved and source_input_kind == "raw_xlsx_zip"
    review_status = build_review_completion_status(
        mismatch,
        load_gyeolsan_review_decisions(),
        base_ready=ready_for_preview,
        high_mismatches=high_mismatches,
        dataset_id=str(report.get("dataset_id") or GYEOLSAN_DATASET_ID),
    )

    if not ready_for_preview:
        reason = "사학재정알리미 통합 결산 원자료 기반 candidate/report가 아직 완성되지 않았습니다."
    elif not review_status.ready_for_promotion:
        reason = "Preview 가능. 단, 운영 CSV와 candidate 차이에 대한 운영자 검토가 필요합니다."
    else:
        reason = "Preview 가능하며 승격 전 검토 기준을 충족했습니다."
...
def build_education_return_validation_status() -> ValidationModeStatus:
    report = load_education_return_processing_report()
    source = load_education_return_source_acquisition()
    candidate_exists = EDUCATION_RETURN_CANDIDATE_PATH.exists()
    report_exists = EDUCATION_RETURN_REPORT_PATH.exists()
    mismatch_exists = EDUCATION_RETURN_MISMATCH_PATH.exists()
    mismatch = load_education_return_mismatch_frame()
    ...
    if not ready_for_preview:
        reason = "사학재정알리미 교육비 환원율 원자료 기반 candidate/report가 아직 완성되지 않았습니다."
    elif not review_status.ready_for_promotion:
        theme_issue_mismatches = int(row_counts.get("theme_issue_operating_mismatch_rows", 0) or 0)
        if theme_issue_mismatches == 0 and row_counts.get("theme_issue_candidate_rows"):
            reason = (
                "사학재정알리미 테마·이슈통계 공식값과 운영 CSV는 차이 0건으로 확인됐습니다. "
                "단, 재무제표 원자료 재계산 candidate와의 차이는 운영자 검토가 필요합니다."
            )
        else:
            reason = "Preview 가능. 단, 운영 CSV와 candidate 차이에 대한 운영자 검토가 필요합니다."
    else:
