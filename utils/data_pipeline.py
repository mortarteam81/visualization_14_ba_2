...
from utils.config import (
    ...
    FACULTY_SECURING_REFERENCE_CSV,
    FACULTY_SECURING_REFERENCE_CSV_ENCODING,
...
def prepare_faculty_securing_reference_frame(
    df: pd.DataFrame,
    *,
    private_only: bool = True,
) -> pd.DataFrame:
    ...
    for column in ["기준년도", *metric_columns]:
        frame[column] = pd.to_numeric(
            frame[column].astype(str).str.replace(",", "", regex=False),
            errors="coerce",
        )
...
def load_faculty_securing_reference_frame() -> pd.DataFrame:
    return prepare_faculty_securing_reference_frame(
        _load_csv(FACULTY_SECURING_REFERENCE_CSV, FACULTY_SECURING_REFERENCE_CSV_ENCODING)
    )
