...
QUOTA_RATE = get_series("fulltime_adjunct_faculty_quota_rate")
ENROLLED_RATE = get_series("fulltime_adjunct_faculty_enrolled_rate")
...
def build_metric(series) -> MetricSpec:
    return MetricSpec(
        ...
        threshold=ThresholdSpec(
            ...
            label=series.threshold_label or "4주기 인증 기준",
            color="#F59E0B",
            dash="dot",
        ),
...
def main() -> None:
    ...
    quota_metric = build_metric(QUOTA_RATE)
    enrolled_metric = build_metric(ENROLLED_RATE)
    ...
    with st.sidebar:
        ...
        criterion = st.radio(
            ...
            index=0,
            help="전임교원 확보율에 겸임교원 확보율 최종 인정값을 더한 값입니다.",
        )
    ...
    selected_metric = quota_metric if criterion == "편제정원 기준" else enrolled_metric
    ...
    render_single_metric_page(
        ...
        selected_schools=selected_schools,
        definition_rows={
            "지표": "전임교원 확보율과 겸임교원 확보율 최종 인정값을 더한 4주기 인증 대응 지표입니다.",
            "편제정원 기준": "편제정원 기준 전임교원 확보율에 편제정원 기준 겸임교원 최종 인정값을 더합니다.",
            "재학생 기준": "재학생 기준 전임교원 확보율에 재학생 기준 겸임교원 최종 인정값을 더합니다.",
            "겸임교원 반영": "기존 겸임교원 확보율 페이지와 동일하게 0.3 가중치와 4% 상한을 적용한 값을 사용합니다.",
            "4주기 인증 기준": "64% 이상",
            "해석 방향": "값이 높을수록 전임교원과 겸임교원을 합산한 교원 확보 수준이 양호한 것으로 해석합니다.",
            "비교 대상 그룹": "선택 학교 외에 그룹 평균과 그룹 구성 학교를 함께 표시해 상대적 위치를 확인합니다.",
            "업데이트": DATA_UPDATED,
        },
        kpi_threshold_suffix="64.0% 이상",
    ...
    render_focus_range_chart(
        ...
        year_col=YEAR_COL,
        school_col=SCHOOL_COL,
        chart_title=f"{selected_metric.label} 기준선 인근 비교",
        chart_styler=chart_styler,
        title="기준선 인근 확대 보기",
        caption="64% 인증 기준 주변 구간을 확대해 학교별 충족 여지를 비교합니다.",
    ...
    render_bump_chart(
        ...
        caption="확보율 순위 변화를 통해 선택 학교와 그룹 평균의 상하위 흐름을 확인할 수 있습니다.",
        toggle_key=f"{PAGE.id}_bump_selected_only",
    )

    render_single_school_metric_comparison(
        filtered_df,
        metrics=[quota_metric, enrolled_metric],
        year_col=YEAR_COL,
        school_col=SCHOOL_COL,
        section_title="기준별 비교",
        chart_title="편제정원 기준 vs 재학생 기준",
        y_label="전임교원 및 겸임교원 확보율 (%)",
        color_map={
            quota_metric.label: "#6EA8FF",
            enrolled_metric.label: "#FF9A4D",
        },
