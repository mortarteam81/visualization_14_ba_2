...
PAGE = get_metric("library_material_purchase")
SERIES = get_series("library_material_purchase_per_student")
...
def build_metric() -> MetricSpec:
    return MetricSpec(
        ...
        threshold=ThresholdSpec(
            value=SERIES.threshold or 54_000.0,
...
def _focus_range(series: pd.Series, metric: MetricSpec) -> tuple[float, float] | None:
    if series.empty:
        return None
    threshold = metric.threshold.value if metric.threshold else 54_000.0
    lower = max(0.0, threshold - 35_000.0)
    upper = threshold + 95_000.0
    if upper <= lower:
        return None
    return lower, upper
...
def main() -> None:
    ...
    render_single_metric_page(
        ...
        selected_schools=selected_schools,
        definition_rows={
            "지표명": "재학생 1인당 연간 자료구입비 결산 규모를 보여주는 교육 지원 지표입니다.",
            "화면 표시값": "재학생 1인당 연간 자료구입비(결산)를 기준 지표로 사용합니다.",
            "산식": "자료구입비계 ÷ 재학생수 (원)",
            "해석 방향": "값이 높을수록 재학생 1인당 도서·전자자료 등 학술자료 구입 투자가 큰 것으로 볼 수 있습니다.",
            "기준값": PAGE.threshold_note,
            "비교 대상 그룹": "선택 학교와 비교 그룹 평균선을 함께 보여줘 상대적 위치를 확인할 수 있습니다.",
            "업데이트": DATA_UPDATED,
        },
        kpi_threshold_suffix=f"{SERIES.threshold:,.0f}원 이상",
    ...
    render_focus_range_chart(
        ...
        chart_styler=chart_styler,
        title="기준값 인근 보기",
        caption="54,000원 기준값 주변 구간을 확대해 학교 간 차이를 더 쉽게 비교할 수 있습니다.",
    ...
    render_comparison_heatmap(
        ...
        selected_schools=selected_schools,
        group_definitions=group_definitions,
        title="학교별 재학생 1인당 연간 자료구입비 히트맵",
        caption="연도별 재학생 1인당 연간 자료구입비 강도를 색으로 보여줘 어느 학교가 높고 낮은지 빠르게 비교할 수 있습니다.",
        hover_value_label="재학생 1인당 연간 자료구입비(원)",
    )
    ...
    render_bump_chart(
        ...
        caption="재학생 1인당 연간 자료구입비 순위 변화를 통해 선택 학교와 비교 그룹 평균의 상대적 흐름을 볼 수 있습니다.",
