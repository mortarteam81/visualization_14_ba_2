...
PAGE = get_metric("scholarship_ratio")
...
def build_metric() -> MetricSpec:
    return MetricSpec(
        ...
        threshold=ThresholdSpec(
            value=SERIES.threshold or 12.0,
...
def _focus_range(series: pd.Series, metric: MetricSpec) -> tuple[float, float] | None:
    ...
    threshold = metric.threshold.value if metric.threshold else 12.0
    lower = max(0.0, min(data_min, threshold - 3.0))
    upper = max(data_max, threshold + 12.0)
    if upper <= lower:
        return None
    return lower, upper
...
def main() -> None:
    ...
    render_single_metric_page(
        ...
        selected_schools=selected_schools,
        definition_rows={
            "지표명": "등록금 수입 대비 장학금 지급 수준을 보여주는 학생 지원 재정 지표입니다.",
            "화면 표시값": "한국대학평가원 대학현황지표의 장학금 비율 재계산값을 우선 사용합니다.",
            "산식": "장학금 지급액 ÷ 등록금 수입 × 100 (%)",
            "해석 방향": "값이 높을수록 등록금 수입 대비 학생에게 환원되는 장학금 규모가 큰 것으로 볼 수 있습니다.",
            "기준값": "12% 이상",
            "비교 대상 그룹": "선택 학교와 비교 그룹 평균선을 함께 보여줘 상대적 위치를 확인할 수 있습니다.",
            "업데이트": DATA_UPDATED,
        },
        kpi_threshold_suffix="12% 이상",
    ...
    render_focus_range_chart(
        ...
        chart_styler=chart_styler,
        title="기준값 인근 보기",
        caption="12% 기준값 주변 구간을 확대해 학교별 장학금 비율의 차이를 비교합니다.",
    ...
    render_comparison_heatmap(
        ...
        selected_schools=selected_schools,
        group_definitions=group_definitions,
        title="학교별 장학금 비율 히트맵",
        caption="연도별 장학금 비율 강도를 색으로 보여줘 어느 학교가 높고 낮은지 빠르게 비교할 수 있습니다.",
        hover_value_label="장학금 비율(%)",
    )
    ...
    render_bump_chart(
        ...
        caption="장학금 비율 순위 변화를 통해 선택 학교와 비교 그룹 평균의 상대적 흐름을 볼 수 있습니다.",
