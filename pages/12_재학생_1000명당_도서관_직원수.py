...
PAGE = get_metric("library_staff")
SERIES = get_series("library_staff_per_1000_students")
...
def _focus_range(series: pd.Series, metric: MetricSpec) -> tuple[float, float] | None:
    ...
    threshold = metric.threshold.value if metric.threshold else 1.0
    lower = max(0.0, min(data_min, threshold - 0.5))
    upper = max(data_max, threshold + 1.5)
    if upper <= lower:
        return None
    return lower, upper
...
def main() -> None:
    ...
    render_single_metric_page(
        ...
        selected_schools=selected_schools,
        definition_rows={
            "지표명": "재학생 1,000명당 도서관 직원 수를 보여주는 학술 지원 인력 지표입니다.",
            "화면 표시값": "R열의 재계산값을 기준 지표로 사용합니다.",
            "산식": "가중 도서관 직원 수 ÷ 재학생수 × 1,000 (명)",
            "해석 방향": "값이 높을수록 재학생 대비 도서관 지원 인력이 상대적으로 충분한 것으로 볼 수 있습니다.",
            "기준값": PAGE.threshold_note,
            "비교 대상 그룹": "선택 학교와 비교 그룹 평균선을 함께 보여줘 상대적 위치를 확인할 수 있습니다.",
            "업데이트": DATA_UPDATED,
        },
        kpi_threshold_suffix=f"{SERIES.threshold:.1f}명 이상",
    ...
    render_focus_range_chart(
        ...
        chart_styler=chart_styler,
        title="기준값 인근 보기",
        caption="1.0명 기준값 주변 구간을 확대해 학교 간 차이를 더 쉽게 비교할 수 있습니다.",
    ...
    render_comparison_heatmap(
        ...
        selected_schools=selected_schools,
        group_definitions=group_definitions,
        title="학교별 재학생 1,000명당 도서관 직원수 히트맵",
        caption="연도별 재학생 1,000명당 도서관 직원수 강도를 색으로 보여줘 어느 학교가 높고 낮은지 빠르게 비교할 수 있습니다.",
        hover_value_label="재학생 1,000명당 도서관 직원수(명)",
    )
    ...
    render_bump_chart(
        ...
        caption="재학생 1,000명당 도서관 직원수 순위 변화를 통해 선택 학교와 비교 그룹 평균의 상대적 흐름을 볼 수 있습니다.",
    ...
    st.markdown("---")
    st.caption(f"데이터 출처: 학술정보통계시스템(RINFO) 가공 CSV | 업데이트: {DATA_UPDATED}")
