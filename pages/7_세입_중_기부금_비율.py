...
def _focus_range(series: pd.Series, metric: MetricSpec) -> tuple[float, float] | None:
    ...
    lower = max(0.0, min(data_min, threshold - 1.0))
    upper = max(data_max, threshold + 1.0)
    if upper <= lower:
        return None
    return lower, upper
...
def main() -> None:
    ...
    render_single_metric_page(
        ...
        selected_schools=selected_schools,
        definition_rows={
            "지표명": "운영수입 대비 기부금수입의 비율을 보여주는 재정 지표입니다.",
            "산식 개요": "기부금수입을 운영수입으로 나눈 뒤 100을 곱해 산출합니다.",
            "해석 방향": "기부금 비율이 높을수록 외부 재원 확보와 재정 다변화 측면에서 긍정적으로 볼 수 있습니다.",
    ...
    render_focus_range_chart(
        ...
        chart_styler=chart_styler,
        title="기준선 인근 확대 보기",
        caption="기부금 비율이 기준선 주변에 있는 학교들끼리 차이를 더 쉽게 볼 수 있도록 확대했습니다.",
    ...
    render_comparison_heatmap(
        ...
        selected_schools=selected_schools,
        group_definitions=group_definitions,
        title="학교별 기부금 비율 히트맵",
        caption="높을수록 좋은 지표 특성을 반영해 낮은 값은 어두운 색, 높은 값은 밝은 색으로 표현했습니다.",
        hover_value_label="기부금 비율(%)",
    )
    ...
    render_bump_chart(
        ...
        caption="기부금 비율은 높을수록 좋은 지표이므로 높은 값일수록 상위 순위로 계산합니다.",
