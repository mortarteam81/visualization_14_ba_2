...
PAGE = get_metric("paper")
PAPER_JAEJI = get_series("paper_jaeji")
PAPER_SCI = get_series("paper_sci")
...
def build_metric(series) -> MetricSpec:
    ...
    if series.id == "paper_jaeji":
        metric_label = "국내 등재지 논문"
        threshold_label = "4주기 인증 기준(국내 등재지)"
        threshold_color = "#38BDF8"
    elif series.id == "paper_sci":
        metric_label = "SCI급·SCOPUS 논문"
        threshold_label = "4주기 인증 기준(SCI급·SCOPUS)"
...
def render_metric_section(
    *,
    ...
    render_focus_range_chart(
        ...
        title="기준선 인근 확대 보기",
        caption="인증 기준 주변 구간을 확대해 학교별 논문 실적 차이를 더 쉽게 볼 수 있습니다.",
    ...
    render_comparison_heatmap(
        ...
        caption="연도별 논문 실적 강도를 색으로 보여주어 선택 학교와 비교 그룹의 흐름을 빠르게 비교할 수 있습니다.",
    ...
    render_bump_chart(
        ...
        caption="연도별 논문 실적 순위 흐름을 통해 선택 학교와 그룹 평균의 상대적 위치를 확인할 수 있습니다.",
...
def main() -> None:
    ...
    group_definitions = build_group_definitions(
        ...
        key_prefix=PAGE.id,
        title="비교 대상 그룹",
        caption="논문 평균선을 함께 보기 위한 비교 그룹을 지정합니다.",
    ...
    render_definition_table(
        {
            "지표": "전임교원 1인당 논문 실적을 국내 등재지와 SCI급·SCOPUS 논문으로 나누어 비교합니다.",
            "국내 등재지 논문": "국내 등재(후보 포함) 학술지 기준 전임교원 1인당 논문 실적입니다.",
            "SCI급·SCOPUS 논문": "국제 학술지 기준 전임교원 1인당 논문 실적입니다.",
